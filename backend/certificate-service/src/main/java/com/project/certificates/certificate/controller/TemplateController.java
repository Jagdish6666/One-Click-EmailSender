...
public class TemplateController {
    ...
    public ResponseEntity<ApiResponse> uploadTemplate(@RequestParam("file") MultipartFile file) {
        try {
            ...
            return ResponseEntity.ok(new ApiResponse(true, "Certificate template uploaded and activated successfully."));
        } catch (IOException e) {
            return ResponseEntity.status(500).body(new ApiResponse(false, "Failed to upload template: " + e.getMessage()));
        }
    }
